...
fn part_two(map: &mut Vec<Vec<bool>>) {
    let (rows, cols) = (map.len(), map.first().unwrap().len());
    let mut num_free_positions = Vec::with_capacity(rows);
    // Initialize, setting the free positions due to the borders
    let mut first = vec![3u8; cols];
    ...
    for (row_num, row) in map.iter().enumerate() {
        for (col_num, col) in row.iter().enumerate() {
            ...
            // add a free position to each of its neighbors
            if !col {
                update_free_positions(&mut num_free_positions, row_num, col_num);
            }
        }
    }
    ...
    while removed_rolls {
        removed_rolls = false;
        // remove rolls until no more can be removed
        for (row_num, row) in map.iter_mut().enumerate() {
            for (col_num, col) in row.iter_mut().enumerate() {
                if *col {
                    if num_free_positions[row_num][col_num] > 4 {
                        if removed_rolls == false {
                            removed_rolls = true;
                        }
                        *col = false;
                        total_removed += 1;
...
fn update_free_positions(num_free_positions: &mut Vec<Vec<u8>>, row_num: usize, col_num: usize) {
    let rows = num_free_positions.len();
    let cols = num_free_positions.first().unwrap().len();
    if row_num > 0 {
        if col_num > 0 {
            num_free_positions[row_num - 1][col_num - 1] += 1;
        }
